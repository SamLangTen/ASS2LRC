...
def display_usage(name):
    print("usage: python " + name + " [--help] -i <inputfile> -o <outputfile> [-t mileseconds] [-s stylename]")
    print("")
...
if __name__ == "__main__":
    ...
    convert_styles = list()
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hi:o:t:s:", ["help", "input=", "output=", "split_timespan=","convert_styles="])
    ...
    if len(convert_styles) != 0:
        ...
        for style_item in dialogues:
            if style_item.style in convert_styles:
